...
export default function FacultySignup() {
  ...
  const handlesubmit = async (e) => {
    e.preventDefault();
    const response = await fetch(`https://hackelite1.herokuapp.com/auth/facultysignup`, {
      method: "POST",
  ...
  return (
    <div className="login">
      <form className="loginbox" onSubmit={handlesubmit}>
        <h1 className="login-title">Faculty Signup</h1>
        ...
        <div className="login-footer-desc">
          <p>
            Already Have An Account? <Link to={"/facultylogin"}> Login</Link>
          </p>
        </div>
      </form>
